...
```bash
npx -y @esncy/canvas-agent --debug
```
...
`canvas-agent` 使用自己的 `package.json` 版本号，不跟仓库根目录 `VERSION` 绑定。推送到 `main` 后，GitHub Actions 会检查 npm 上是否已经存在当前包版本；不存在时才发布 `@esncy/canvas-agent`。
...
直接运行 `npx -y @esncy/canvas-agent` 只启动本地 Agent 服务，不会安装 MCP，也不会增加 Codex 工具上下文。只有安装 Codex app 插件，或手动执行 `codex mcp add` 后，`infinite-canvas` 工具才会进入 Codex 上下文；由于工具较多，不使用时建议移除。
...
仓库内提供了 Codex app 插件：`plugins/infinite-canvas`。在 Codex app 中添加本仓库的 marketplace 后，可以安装“大狗画布”插件；插件会注册同一个 `infinite-canvas` MCP，并带上画布操作说明。
...
```bash
npx -y @esncy/canvas-agent mcp
```

使用时可以直接在 Codex 里说“打开大狗画布”，插件会启动本地 Agent，读取 Local URL 和 Connect token，然后在右侧打开本地测试站并自动新建、连接画布；只有明确要求使用本地项目时才会启动本地前端。

Canvas Agent 启动后，给 Codex 添加 MCP：

```bash
codex mcp add infinite-canvas -- npx -y @esncy/canvas-agent mcp
```
...
```bash
claude mcp add --scope user --transport stdio infinite-canvas -- npx -y @esncy/canvas-agent mcp
```
